...
public class i_test {
    ...
    // 1. 테스트 코드란?  (다른걸 할줄 알때  이것을 하면 취업에 큰강점이 될수도 있다)
    // : 애플리케이션이 의도한 대로 작동하는지 확인하기 위해 작성된 "코드"
    // : 개발자가 직접 코드의 결과를 수작업으로 확인하는 대신
    //   , "자동화된 테스트"로 특정 기능이 올바르게 동작하는지를 반복적으로 확인

    // 2. 테스트 코드의 종류
    // : 단위 테스트, 통합 테스트(여러 모듈  MVC), 기능 테스트(사용자관점), 회귀 테스트(코드작성 끝난후 코드동작 유지확인)
    ...
    // 스프링 부트 스타터 테스트 목록
    // - JUnit 5: 자바  프로그래밍 언어용 단위 테스트 프레임워크(작은 단위로 나누고 나눠 용)
