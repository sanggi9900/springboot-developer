...
public class j_ORM {
    ...
    // : 객체와 관계형 데이터 베이스 간의 데이터를 매핑해주는 기술
    ...
    // - Field: 클래스와 필드는 테이블의 열(column)에 해당

    // 2. JPA(Java Persistence(영속성) API)
    // : ORM의 종류 중 하나
...
//    }
    ...
    // : 기본 키 자동 생성 전략 설정
    // +) 다양한 옵션이 있음
    //    - strategy옵션: GenerationType.IDENTITY(auto-increment)

    // @Column
    // : 필드를 특정 테이블 열과 매핑
    // : 생략 시 기본으로 필드 이름이 열이름으로 사용
    // +) 다양한 옵션이 있음
    //    - name 옵션: 열 이름 지정
