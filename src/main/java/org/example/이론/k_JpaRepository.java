...
public class k_JpaRepository {
    ...
    // 2) findById(Id id)
    // - 주어진 ID에 해당되는 엔터티를 조회

    // 3) existsById(ID id): boolean   // 있는확인 한 뒤 데이터를 가져온다
    // - 주어진 ID가 존재하는지 확인
    ...
    // EX1) '가격'이 특정 금액 '이상'인 상품 '조회'   // 인자값을 전달해야 매개변수로 사용한다
...
//    findByPriceGreaterThan(1000)
    ...
    // @Query("SELECT * FROM Product WHERE p.price > :price")
