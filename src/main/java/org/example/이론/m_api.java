...
public class m_api {
    ...
    // ==== 1. API  // 서버와 클라이언트 연결  어디서든 사용가능 ==========
    // : Application Programming Interface
    // : 응용프로그래밍(Application)이 서로 통신할 수 있도록 해주는 인터페이스
    // : 소프트웨어 간의 중재자 역활
    ...
    // ====  2. HTTP (API) // WEB과 사용자간의 소통(연결) =========
    // : 웹 환경에서 정보를 주고받기 위해 프로토콜
    // : HTTP라는 통신 규치긍로 소통하는 API
    ...
    // : HTTP 프로토콜 기반으로 클라이언트와 서버 간의 상호작용을 규정하는 아키텍쳐 스타일

    // cf) 아키텍쳐 스타일
    //      : 네트워크 자원을 정의하고 처리하는 방법 전반

    // : "자원"의 "표현"으로 "상태"를 전달하는 아키텍쳐

    // REST의 6가지 원칙
    // 1) 클라이언트-서버 구조: 클라이언트와 서버가 명확하게 분리
    // 2) 무상태성: 서버는 클라이언트의 상태 저장 X, 요청은 독립적으로 처리
    // 3) 캐시 가능성: 응답을 캐시될 수 있어야 함  (요청결과를 저장장한다)
    ...
    // cf) URI가 URL을 포함
    // - URI는 통합 지원 식별자로 주소에 식별자가 있으면 URI  (경로의 데이터를 원한다)
    // - URL은 리소스(자원)의 주소를 나타냄 (경로)

    // 자원: URI로 자원을 표현
    // 행위: 자원에 대한 행위는 HTTP Method로 표현
    // 표현: 응답 자원의 상태를 JSON, XML 등의 형태로 나타냄

    // ======= 4. RESTful(형용사 꾸며주는 접미사) API ====
    ...
    // createProduct   (동사)  상품을 생성해라 생성하다
    // createdProduct  (명사)  생성된 상품

    // 2. 동사는 HTTP 메소드 사용 (CRUD - 크루드)
    // POST: 만들고(CREATE)
    // GET: 읽고(READ)
    // PUT: 업데이터하고(UPDATE)
    // DELETE: 삭제하고(DELETE)
    ...
    // >> 리소스 일부만 제공, 제공되지 않는 정보는 null 또는 기본값으로 설정
    // >> 멱등성을 가짐(Idempotent)
