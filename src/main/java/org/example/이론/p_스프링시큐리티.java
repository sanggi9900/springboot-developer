...
public class p_스프링시큐리티 {
    ...
    // : 스프링 기반 애플케이션 보안(인증, 인가, 권한)을 담당하는 스프링 하위 프레임워크
    // >> 다양한 애너테이션으로 CSRF 공격, 세션 고정 공격을 방어
    // >> 요청 헤더 보안 처리도 가능

    // 1. 인증(Authentication - auth  어텐틱케이션)
    // EX) 시스템이 사용자가 제시한 증거(예: 아이디, 비밀번호)를 바탕으로
    //      , 그 사용자가 주장하는 인물임을 확인

    // 2. 인가(Authorization - auth 어또리제이션)
    ...
    // 3. 유효성 검사 완료 시 실제 구현체인 UsernamePasswordAuthenticationToken을 만드렁 전달
    //      : 전달받은 인증용 객체를 "AuthenticationManager"에게 전달

    // 4. UsernamePasswordAuthenticationToeken을 AuthenticationProvider에 전달
    ...
    // 7. 입력 정보오 UserDetails 정보를 비교해 실제 인증 처리를 진행
